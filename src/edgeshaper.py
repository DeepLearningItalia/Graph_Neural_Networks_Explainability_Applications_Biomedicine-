...
class Edgeshaper():
    ...
    def compute_minimal_top_k_set(self, verbose = False):
        ...
        for i in range(important_edges_ranking.shape[0]):
            ...
            # print(out_prob)
            predicted_class = torch.argmax(out_prob[0]).item()
            # print("in compute minimal top k set:")
            # print("predicted class: ", predicted_class)
            # print("target class: ", self.target_class)
            # print("out prob: ", out_prob)
            # print("original pred prob: ", self.original_pred_prob)
            if predicted_class != self.target_class:
                pred_prob = out_prob[0][self.target_class].item()
    ...
    def visualize_molecule_explanations(self, smiles, save_path=None, pertinent_positive = False, minimal_top_k = False):
        ...
        if minimal_top_k:
            ...
            for i in range(pertinent_set_edge_index.shape[1]): #TODO CORRECT IN EDGESHAPER AND PLI REPO change 0 to 1. In protocol branch change and maybe rerun
